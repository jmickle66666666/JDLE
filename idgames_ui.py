...
class Idgames_wad_UI(Tk):
    ...
    def download_wad(self):
        ...
        print(url)
        r = requests.get(url)
        if (os.path.isdir(JDLE_DIR+"/download") == False):
            os.makedirs(JDLE_DIR+"/download")
        f = open(JDLE_DIR+"/download/"+self.data["filename"],"wb")
        f.write(r.content)
        f.close()
        print("done")
        self.open_wad()
...
class Idgames_UI(Tk):
    ...
    def load_latest(self):
        ...
        def async_latest():
            json = idgames.latestfiles(limit=15)
            self.results_box.delete(0,"end")
            self.result_data = {}
            for f in json["content"]["file"]:
                self.results_box.insert("end",f["title"])
                self.result_data[f["title"]] = f["id"]
    ...
    def idgames_search(self):
        ...
        def async_search():
            json = idgames.search(self.search_bar.get())
            self.results_box.delete(0,"end")
            if (json["content"]["file"] is list):
                self.result_data = {}
                for f in json["content"]["file"]:
                    self.results_box.insert("end",f["title"])
                    self.result_data[f["title"]] = f["id"]
            else:
                self.results_box.insert("end",json["content"]["file"]["title"])
                self.result_data = {}
                self.result_data[json["content"]["file"]["title"]] = json["content"]["file"]["id"]
    ...
    def on_result_click(self,event):
        ...
        if (value == self.last_value):
            Idgames_wad_UI(None,self,idgames.get(id=self.result_data[value]))
        else:
            self.last_value = value
    
    def random_wad(self):
        json = idgames.latestfiles(limit=1)
        lastid = json["content"]["file"]["id"]
        randid = random.randrange(lastid)
        Idgames_wad_UI(None,self,idgames.get(id=randid))
